...
const size_t k_max_msg = 4096; // costante per definire la dimensione massima

static int32_t one_request(int connfd)
{
    // 4 bytes header
    char rbuf[4 + k_max_msg + 1]; // dichiara la dimensione del buffer 4byte + k_max_msg + 1 byte null per finire la stringa
    errno = 0;
    int32_t err = read_full(connfd, rbuf, 4); // leggi i primi 4 byte della connessione e memorizzali nel buffer
    if (err)
    {
        if (errno == 0)
        {
